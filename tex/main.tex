...
or more presumed hard problems.
...
%
some \emph{rewrites} (also called a \emph{hops} or \emph{transitions}) result
in a game that is semantically equivalent, in which case no attacker can
distinguish them;
%
others result in distinguishable events that occur only with some small
probability, such as a collision in the range of a random oracle;
%
still other rewrites are bounded computationally, by reducing a distinguisher
to breaking some underlying assumption, such as the hardness of solving
discrete logarithms on the elliptic curve used by the protocol.
...
%
While steps like this are significant, the provable security methodology has some
...
\begin{enumerate}
  ...
  \item Proofs are most often written in some \emph{pseudocode} language
    ...
    10 papers or more to review, each with its own unique and in possibly
    compatible semantics.

  \item Protocols of interest to IETFers and their security goals are becoming
    more complex.
    %
  ...
  \item Mechanization of security proofs can help reduce burden on human
    reviewers.
    %
    Several competing tools exist for this, including
...
\end{enumerate}
%
These factors put our community on a trajectory that, in the author's view, is
...
\paragraph{Proposal.}
%
This essay puts forward a vision for writing security proofs for protocols
developed at IETF.
%
...
We believe these proofs should meet two basic requirements.
%
First, proofs must be immediately amendable to mechanized verification. This is
critical to minimize the cost of verifying proofs as protocols become more
sophisticated.
%
Second, generating proofs must not require significantly more effort or
expertise than pen-and-paper.
%
\cp{...}

% XXX To meet these requirements, we envision a \emph{separation of concerns}
% between the prover and the verifier.
%
% This is probably a not a new idea, but we're now at a point where it's more
% likely to succeed.
...
to some \emph{ideal functionality} and the attacker's goal is to distinguish
between the real system and a simulator that has access to the ideal
functionality.
%
There is no fundamental difference between simulation- and game-based
definitions that makes mechanization of one harder than the other. However, the
details of popular simulation frameworks, like UC framework~\cite{Can01}, can
be rather involved. Nevertheless, proofs in this framework have been
mechanized~\cite{CSV19}.


\paragraph{What about symbolic analysis?}
%
Tools like \href{https://tamarin-prover.com/}{Tamarin} or
\href{https://bblanche.gitlabpages.inria.fr/proverif/}{ProVerif} are useful for
proving \emph{symbolic} security properties of protocols rather than
computational ones.
%
% XXX However, expressing security properties ...
