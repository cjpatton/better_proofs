...
\emph{game} played by the attacker.
%
A security proof relates the attacker's advantage in winning the game to the
difficulty of one or more presumed hard problems.

Games are often expressed in \emph{code} rather than natural language, making
it amendable to the following proof strategy.
%
The proof consists of a sequence of games, sometimes called \emph{hybrids}.
Beginning with the real game, each game is obtained by modifying the code of
the previous one. In the final game, typically the attacker interacts with some
object that models the ideal interaction of uncorrupted parties.

The prover's job is to establish, for each pair of neighboring games, that the
probability of the attacker in distinguishing between the games is negligible.
%
Sometimes the rewrite results in a game that is semantically equivalent, in
which case the attacker gets no advantage.
%
Some rewrites are fixed information theoretically, by enumerating the events
that would result in distinguishable behavior and analyzing each event's
probability.
%
Other rewrites must be established computationally, by reducing
distinguishability to some computational assumption.

In recent years, security proofs of this sort have played an increasingly
central role in the design and analysis of protocols specified at IETF. In
particular, we have seen working groups move from \emph{reacting} to knew
attacks to \emph{proactively} seeking security analysis during the design
phase~\cite{KT16}.
%
We have even seen this process formalized in the TLS working
group~\cite{tls13-formal-analysis-triage-panel}.
%
Steps like this are significant, but the provable security methodology has some
well-known limitations~\cite{Ber19}.
%
...
and Rogaway assessed that many proofs of the era were ``essentially
unverifiable'', meaning errors in these proofs, some of which could result in
attacks, may go unnoticed. They feared that provable security would
reach a ``crisis of rigor'' in which proofs were believed without being fully
vetted.
%
This fear has come to fruition, at least partially: several attacks have been
documented that were missed due to a bug in the initial analysis.
%
\cp{TODO Add references for some of these.}
%
Bellare and Rogaway hoped that game playing would help reduce this risk.

Today carefully checking computational security proofs of any type, including
game-based proofs, remains a significant challenge. There seem to be at least three
...
\begin{enumerate}
  ...
  \item Proofs are most often written in some \emph{pseudocode} language
    ...
    proofs.
    %
    There is no ``standard'' pseudocode in use today: the syntax and semantics
    are as varied as the communities that use them.
    %
    This can make life very difficulty for program committee members, who may
    have a stack of 10 papers or more to review, each with its own unique and
    in possibly compatible semantics.

  \item Although pen-and-paper proofs are virtually impossible to check
    mechanically (pseudocode typically has incomplete semantics), several
    competing tools exist for developing mechanized proofs, such as
    ...
    %
    There are several reasons for this: maturity of the tooling; the
    requirement to learn a domain specific language; or the tools being
    tailored to specific (classes of) applications.
    %
    \cp{This seems like a controversial take: I would like to hear from others
    on this point.}

  \item Protocols of interest to practitioners at IETF and beyond are becoming
    more complex.
    %
    First, we are seeing communities do more with cryptography, often going
    beyond well-studied goals like key exchange, authentication, and
    encryption.
    %
    Second, as attacks get more sophisticated, so must security definitions
    model the execution environment in greater detail.
    %
    The net effect of these forces is that proofs are getting \emph{longer}.
    This puts pressure on paper submitters, who usually need to respect a
    strict page limit in order to get their paper accepted at a conference or
    journal.
    %
    This forces them either to replace the full proof with a ``sketch'' or punt
    it to the paper's appendix. Typically, program committee members are not
    required to review.
    %
    Given this trajectory of applied cryptography research and the process for
    reviewing it, something has to change in order to ensure proofs are
    thoroughly vetted.
...
\paragraph{Thesis.}
%
\cp{Mechanization is required; need a separation of concerns between subject
matter experts and formal methods experts.}

\paragraph{What this write up is about.}

\paragraph{Scope.}
%
\cp{Another way to define and prove computational security properties is the
simulation paradigm...}
\cp{Talk about symbolic analysis.}
...
random permutation and a random function over the same domain and range.
%
