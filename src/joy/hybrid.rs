//! Hybrid public-key encryption scheme in Chapter 16.
...
// Chapter 16: Public-Key Encryption
//
// Syntax

/// Public-key encryption.
pub trait PubEnc: Clone {
...
/// Game 0.
pub type G0<P, S> = PubCpa<Hybrid<P, S>>;

/// Game 1: Unroll encryption and key generation and add code for generating another
/// temporary key. Only the first is used. Call the first `tk_left` and the second
/// `tk_right`.
///
/// Claim: For all distinguishers `adv`,
///
/// ```text
/// Pr[adv.play(G0::init(hybrid, true))] = Pr[adv.play(G1::init(hybrid, true))]
/// ```
    ...
    Standard: Distribution<S::Key>,
{
    ...
    fn left_or_right(
        ...
        let tk_left = thread_rng().gen();
        let _tk_right = thread_rng().gen();
...
/// Game 2: Encrypt the plaintext with `tk_right` instead of `tk_left`.
///
/// Claim: XXX
